...
function ResetAllFn() {
    ...
    var _t = localStorage.getItem("_t");
    var loaderEnable = localStorage.getItem("loaderEnable");
    localStorage.clear();
    localStorage.setItem("loaderEnable", loaderEnable);
    localStorage.setItem("_t", _t);
    ...
    sections.forEach((elem) => {
        ...
        if (
            ...
            if (elem.closest(".child1")?.previousElementSibling) {
                elem.closest(".child1").previousElementSibling.classList.add(
                    "active"
                );
            }
        } else {
